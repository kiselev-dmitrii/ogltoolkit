...
void LightScene::init() {
        ...
        // Модели
        m_ogre = new Entity("meshes/model.obj");
        m_ogre->rotate(vec3(90, 0, 0));

        // Загружаем текстуру
        m_tuBase = new TextureUnit();
        m_texOgre = new Texture2D("images/ogre_diffuse.png");

        m_tuBase->bind();
        m_texOgre->bind();

        // Настройка нескольких источников света
        setLightSource(0, vec3(-10,-10,6), vec3(1.0), vec3(1,1,1), vec3(1,1.0,1.0));
