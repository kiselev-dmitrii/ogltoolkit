...
void ShadowmapTest::initEntities() {
        ...
        m_entityMgr->createEntity("sphere", "sphere.mesh")->setNode(m_node4);
        m_node4->setScaleInParent(vec3(0.5, 0.5, 0.5));
...
void ShadowmapTest::update(float deltaTime) {
        ...
        if(Keyboard::isKeyPressed('=')) m_spot->setCutoffAngle(m_spot->cutoffAngle()-0.3);
        if(Keyboard::isKeyPressed('O')) m_mainCamera->setProjectionType(ProjectionType::ORTHOGONAL);
        if(Keyboard::isKeyPressed('P')) m_mainCamera->setProjectionType(ProjectionType::PERSPECTIVE);
